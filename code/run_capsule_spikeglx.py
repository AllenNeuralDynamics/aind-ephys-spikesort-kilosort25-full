...
if __name__ == "__main__":
    ...
    try:
        ...
        ## SpikeGLX data loader ##
        input_format = "spikeglx"
        ecephys_spikeglx_folders = [p for p in data_folder.iterdir() if p.is_dir()]
        assert len(ecephys_spikeglx_folders) == 1, "Provide one SpikeGLX folder at a time"
        ecephys_spikeglx_folder = ecephys_spikeglx_folders[0]
        ...
        for stream_name in stream_names:
            # only proess AP streams
            if "ap" in stream_name:
                recording = se.read_spikeglx(ecephys_spikeglx_folder, stream_name=stream_name)
                ...
                for i_r, recording in enumerate(recordings):
                    if CONCAT:
                        ...
                        recording_name = f"{stream_name}_recording{i_r + 1}"
